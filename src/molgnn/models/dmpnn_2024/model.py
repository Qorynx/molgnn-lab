...
class DMPNN(BaseMolecularModel):
    ...
    required_batch_fields = ("x", "edge_index", "edge_attr", "reverse_edge_index", "batch")
    ...
    def __init__(
        ...
        dimensions = [hidden_dim] + [ffn_hidden_dim] * ffn_num_hidden_layers + [num_targets]
        self.ffn_layers = nn.ModuleList(
            [nn.Linear(input_dim, output_dim) for input_dim, output_dim in pairwise(dimensions)]
        )
    ...
    def forward(self, batch: Batch) -> Tensor:
        ...
        for layer in self.ffn_layers[:-1]:
            hidden = F.dropout(F.relu(layer(hidden)), p=self.dropout, training=self.training)
    ...
    def fingerprint(self, batch: Batch) -> Tensor:
        ...
        x, edge_index, edge_attr, reverse_edge_index, graph_batch = self._batch_tensors(batch)
    ...
    def _batch_tensors(self, batch: Batch) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        if reverse_edge_index.shape != (edge_count,) or reverse_edge_index.dtype != torch.long:
            raise ValueError("batch.reverse_edge_index must have shape [E] and dtype torch.long")
        if graph_batch.ndim != 1 or graph_batch.shape[0] != x.shape[0]:
            raise ValueError("batch.batch must have shape [N]")
        if graph_batch.dtype != torch.long or graph_batch.numel() == 0 or graph_batch.min() < 0:
            raise ValueError("batch.batch must contain non-negative torch.long graph indices")
        ...
        if edge_count and not torch.equal(edge_index[:, reverse_edge_index], edge_index.flip(0)):
            raise ValueError("batch.reverse_edge_index must map each edge to its reverse")
