...
def _smiles_proxy(data: MolecularData, *, sample: int | str) -> tuple[Tensor, Tensor]:
    ...
    if not isinstance(x, Tensor) or x.ndim != 2 or x.shape[0] < 1:
        raise TransformError(f"sample {sample} has invalid x for a GemNet conformer proxy")
...
def _embed_heavy_atom_conformer(
    ...
    parameters.randomSeed = 0x6E37
    if AllChem.EmbedMolecule(molecule_h, parameters) != 0:
        raise TransformError(f"sample {sample} could not embed a GemNet conformer proxy")
    try:
...
def _validate_native_inputs(
    ...
    if not isinstance(atomic_number, Tensor):
        raise TransformError(f"sample {sample} requires atomic_number for native GemNet geometry")
    if (
